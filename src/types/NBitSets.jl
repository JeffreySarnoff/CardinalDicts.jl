...
function Base.getindex(bitset::NBitSet{N}, index::I) where I where N
    ...
    return one(Int16) === (bitset.value[offset+one(Int16)] >> (15%Int16 - bitidx%Int16)) & one(Int16)
end
...
function Base.setindex!(bitset::NBitSet{N}, value::Bool, index::I) where I where N
    ...
    offset, bitidx = fldmod(index%Int16, 16%Int16)
    if value
        bitset.value[offset+one(Int16)] = bitset.value[offset+one(Int16)] | (one(Int16) << bitidx)
    end
    return bitset
end
